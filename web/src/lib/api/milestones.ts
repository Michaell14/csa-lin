...
export async function createMilestone(sb: Supabase, input: { lin_id: string; title: string; event_date: string; description: string | null; created_by: string }): Promise<void> { const { error } = await sb.from('lin_milestones').insert(input); if (error) throw error }
