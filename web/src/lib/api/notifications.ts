...
export async function markAllRead(sb: Supabase): Promise<void> { const { error } = await sb.from('notifications').update({ read_at: new Date().toISOString() }).is('read_at', null); if (error) throw error }
