        ...
        <div aria-label="Lin view" className="ml-auto flex shrink-0 rounded-md bg-surface-hover p-0.5 text-sm">
          {VIEWS.map(([key, name]) => (
            <button key={key} onClick={() => onView(key)} aria-pressed={view === key}
              className={`relative rounded px-1.5 py-1 transition-[background-color,color,box-shadow] duration-100 after:absolute after:inset-x-0 after:-inset-y-1.5 after:content-[''] sm:px-2.5 ${view === key ? 'bg-white font-medium text-ink shadow-border' : 'text-ink-body hover:text-ink'}`}>{name}</button>
          ))}
        </div>
      </div>
    </div>
