...
const linkApi = vi.hoisted(() => ({ pendingRemovalLinkIds: vi.fn<() => Promise<Set<string>>>() }))
vi.mock('@/lib/api/links', () => ({ pendingRemovalLinkIds: linkApi.pendingRemovalLinkIds }))
...
describe('SidePanel own-profile gating', () => {
  beforeEach(() => {
    hookCalls.length = 0
    linkApi.pendingRemovalLinkIds.mockReset().mockResolvedValue(new Set())
  })
  ...
  it('enables removal again when a refreshed request is no longer pending', async () => {
    state.personId = 'me'
    linkApi.pendingRemovalLinkIds.mockResolvedValueOnce(new Set(['link-1'])).mockResolvedValueOnce(new Set())
    ...
    const remove = screen.getByRole('button', { name: 'Request removal of Derek Zhang' })
    await waitFor(() => expect(remove).toBeDisabled())
    fireEvent.focus(window)
    await waitFor(() => expect(remove).toBeEnabled())
    expect(remove).toHaveTextContent('Request removal')
  })
