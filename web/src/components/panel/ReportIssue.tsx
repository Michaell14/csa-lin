...
export function ReportIssue({ personId }: { personId: string }) {
  ...
  const [kind, setKind] = useState<Extract<CorrectionKind, 'profile' | 'relationship'>>('profile')
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="link text-sm">Suggest a correction</button>
