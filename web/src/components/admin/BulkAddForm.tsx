...
export function BulkAddForm({ onAdd }: { onAdd: (rows: NewPerson[]) => Promise<void> }) {
  ...
  return (
    <div className="card flex flex-col gap-3 p-5 text-sm">
      <p className="display text-lg tracking-[-0.02em]">Bulk add from a spreadsheet</p>
      ...
      <div className="flex gap-2">
        ...
        {preview && preview.rows.length > 0 && <button type="button" onClick={add} className="btn-sm-accent">Add {preview.rows.length} people</button>}
      </div>
