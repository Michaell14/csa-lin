...
vi.mock('@/lib/api/people', () => ({ fetchPeopleByIds }))
vi.mock('@/lib/viewer', () => ({
...
const report = (over: Record<string, unknown> = {}) => ({
  id: 'r1', person_id: 'p4', kind: 'profile', details: 'Grad year is wrong.',
  status: 'pending', created_at: '2026-09-13T00:00:00Z', ...over,
})

describe('CorrectionsAdmin', () => {
  beforeEach(() => {
    listCorrections.mockReset(); resolveCorrection.mockReset(); fetchPeopleByIds.mockReset()
  ...
  it('names the person each report concerns', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Child One')).toHaveLength(2))
  ...
  it('says so when a report names nobody yet', async () => {
    ...
    render(<CorrectionsAdmin />)
    expect(await screen.findByText('Nobody on the lin yet')).toBeInTheDocument()
  })
