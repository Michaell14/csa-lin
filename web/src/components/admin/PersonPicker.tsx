...
export function PersonPicker({ label, value, onPick }: { label: string; value: PersonHit | null; onPick: (h: PersonHit | null) => void }) {
  ...
  return (
    <div className="flex flex-col gap-1 text-sm">
      ...
      {value
        ? <span className="flex h-10 items-center gap-2 font-bold">{value.display_name} &#39;{String(value.grad_year).slice(-2)} <button onClick={() => { onPick(null); setKey(k => k + 1) }} className="link">change</button></span>
