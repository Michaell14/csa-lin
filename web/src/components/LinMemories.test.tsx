...
it('removes a deleted memory and its confirmation even when the refresh fails', async () => {
  mocks.fetch.mockResolvedValueOnce([memory('dinner')]).mockRejectedValueOnce(new Error('Timeline unavailable'))
  const user = userEvent.setup()
  render(<LinMemories lin={lin} />)
  await user.click(await screen.findByRole('button', { name: 'Memory actions' }))
  await user.click(screen.getByRole('menuitem', { name: 'Delete memory' }))
  await user.click(screen.getByRole('button', { name: 'Delete' }))
