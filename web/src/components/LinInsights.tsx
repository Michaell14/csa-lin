...
export function LinInsights({ graph, linId }: { graph: LinGraph; linId: string }) {
  ...
  return <div className="h-full overflow-y-auto bg-cream px-4 py-6 sm:px-8"><div className="mx-auto max-w-4xl space-y-8">
    <section><h2 className="display text-lg">Lin at a glance</h2><div className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-4">{cards.map(([label, value]) => <div key={label} className="card p-4"><p className="display text-2xl">{value}</p><p className="text-sm text-ink-muted">{label}</p></div>)}</div></section>
    <section className="card p-4"><h2 className="display text-base">Members by class year</h2><div className="mt-4 space-y-2">{stats.years.map(([year, count]) => <div key={year} className="grid grid-cols-[4rem_1fr_2rem] items-center gap-2 text-sm"><span>{year}</span><span className="h-3 overflow-hidden rounded-full border-2 border-ink bg-white"><span className="block h-full bg-accent" style={{ width: `${count / max * 100}%` }} /></span><span className="text-right font-bold text-ink-muted">{count}</span></div>)}</div></section>
    <section className="card p-4"><h2 className="display text-base">Recorded history</h2>{timeline.length ? <ol className="mt-3 border-l-2 border-ink pl-4">{timeline.map(year => <li key={year.label} className="relative pb-4 last:pb-0"><span className="absolute -left-[1.34rem] top-1 h-2.5 w-2.5 rounded-full border-2 border-ink bg-gold" /><p className="font-bold">{year.label}</p><p className="text-sm text-ink-muted">{year.count} {year.count === 1 ? 'family link' : 'family links'} recorded</p></li>)}</ol> : <p className="mt-2 text-sm text-ink-muted">Academic years haven’t been recorded for this lin yet.</p>}</section>
    {milestoneError && <p role="alert" className="error">Could not load community milestones: {milestoneError}</p>}
    {milestones.length > 0 && <section className="card p-4"><h2 className="display text-base">Community milestones</h2><ol className="mt-3 space-y-4">{milestones.map(item => <li key={item.id}><p className="font-bold">{item.title}</p><p className="text-xs text-ink-muted">{new Date(`${item.event_date}T00:00:00`).toLocaleDateString()}</p>{item.description && <p className="mt-1 text-sm text-ink-body">{item.description}</p>}</li>)}</ol></section>}
  </div></div>
}
